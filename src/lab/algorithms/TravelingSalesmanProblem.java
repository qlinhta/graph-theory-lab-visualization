...
public class TravelingSalesmanProblem {
	...
	public static void nearestNeighbor(AdjacencyListGraph graph, ConsumerWithException<Edge> onUnvisitedEdge,
			ConsumerWithException<Node> onVisitedNode, ConsumerWithException<Edge> onVisitedEdge,
			Consumer<Node> onProcessedNode, Boolean isCanceled) throws Exception {

		Node startingNode = GraphUtils.getStartingNode(graph);
		Node current = startingNode;
		Set<Node> visitedNodes = new HashSet<>();

		nn(graph, current, startingNode, visitedNodes, onUnvisitedEdge, onVisitedNode, onVisitedEdge, onProcessedNode,
				isCanceled);
	}

	private static boolean nn(AdjacencyListGraph graph, Node current, Node startingNode, Set<Node> visitedNodes,
			ConsumerWithException<Edge> onUnvisitedEdges, ConsumerWithException<Node> onVisitedNode,
		...
		for (Edge edge : edges) {
			...
			if (!visitedNodes.contains(destination)
					|| (visitedNodes.size() == graph.getNodes().size() - 2 && destination.equals(startingNode))) {
				onVisitedEdge.accept(edge);
				visitedNodes.add(destination);
				destination.setPathParent(current);
				if (nn(graph, destination, startingNode, visitedNodes, onUnvisitedEdges, onVisitedNode, onVisitedEdge,
						onProcessedNode, isCanceled)) {
					return true;
				}
	...
	public static void twoOpt(AdjacencyListGraph graph, ConsumerWithException<Edge> onUnvisitedEdge,
			ConsumerWithException<Node> onVisitedNode, ConsumerWithException<Edge> onVisitedEdge,
			Consumer<Node> onProcessedNode, Boolean isCanceled) throws Exception {
	...
	private static void unsetPath(Edge edge1, Edge edge2, ConsumerWithException<Edge> onUnvisitedEdge)
			throws Exception {
	...
	private static boolean firstValidRoute(AdjacencyListGraph graph, Node current, Node startingNode, List<Node> path)
			throws Exception {
		...
		for (Edge edge : edges) {
			...
			if (!path.contains(destination)
					|| (path.size() == graph.getNodes().size() && destination.equals(startingNode))) {
