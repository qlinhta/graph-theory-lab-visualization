...
public class ShortestPath {
	...
	public static void dijkstra(Graph graph, ConsumerWithException<Node> onVisitedNode,
		...
		PriorityQueue<Node> toBeVisitedNodes = new PriorityQueue<>(
				(n1, n2) -> Integer.compare(n1.getPathCost(), n2.getPathCost()));
	...
	public static void bellmanFord(Graph graph, ConsumerWithException<Node> onVisitedNode,
			ConsumerWithException<Edge> onVisitedEdge, Consumer<Node> onProcessedNode,
		...
		for (int j = 0; j < graph.getNodes().size() - 1; j++) {
			...
			for (Edge edge : edges) {
				...
				// only the first time draws the edges, otherwise it takes too much to draw the
				// interf
				if (j == 0) {
