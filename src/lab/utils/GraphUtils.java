...
public class GraphUtils {
			...
			for (int k = 0; k < edgesNumber; k++) {
				if (random.nextBoolean()) {
					...
					for (int j = 0; j < nodes.size(); j++) {
						...
						if (i == j || IntStream.of(selectedEdges).anyMatch(x -> x == jj))
							continue;
	...
	public static int getDistance(Node start, Node end) {
		return (int) (Math.sqrt(
				Math.pow(Math.abs(start.getX() - end.getX()), 2) + Math.pow(Math.abs(start.getY() - end.getY()), 2)));
	}

	public static Node getStartingNode(Graph graph) throws Exception {
		return graph.getNodes().stream().filter(Node::isStartNode).findFirst()
				.orElseThrow(() -> new Exception("Starting node not present."));
	}

	public static Node getTargetNode(Graph graph) throws Exception {
		return graph.getNodes().stream().filter(Node::isTargetNode).findFirst()
				.orElseThrow(() -> new Exception("Target node not present."));
	}
}
